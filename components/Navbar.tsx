  ...
  return (
    <div className="pointer-events-none absolute left-1/2 top-4 z-[1000] w-[calc(100%-1rem)] max-w-[calc(100%-1rem)] -translate-x-1/2 px-1 md:left-[62.5%] md:w-[calc(78%-1.5rem)] md:max-w-[52rem]">
      <div className="pointer-events-auto flex flex-col gap-2 rounded-[2rem] bg-[#222222]/95 px-3 py-2 text-white shadow-[0_18px_40px_rgba(0,0,0,0.28)] ring-1 ring-black/10 backdrop-blur sm:flex-row sm:items-center sm:justify-between">
        <div ref={searchRef} className="relative sm:order-2 sm:w-[15rem] md:w-[18rem]">
          <p className="mb-1 hidden px-1 text-[10px] font-medium uppercase tracking-[0.18em] text-white/35 sm:block">
          ...
          <input
            ...
            className="w-full rounded-full border border-white/10 bg-white/5 px-4 py-2 text-sm text-white outline-none transition placeholder:text-white/35 focus:border-white/25"
          />
          ...
          {isOpen && (results.length > 0 || query.trim().length >= 2) ? (
            <div className="absolute left-0 right-0 top-[calc(100%+0.5rem)] z-[1010] overflow-hidden rounded-2xl border border-white/10 bg-[#171717] shadow-2xl">
              {results.length > 0 ? (
                results.map((place) => (
                  <button
        ...
        <div className="sm:flex-1">
          <p className="mb-1 px-1 text-[10px] font-medium uppercase tracking-[0.18em] text-white/35">
            Refine the map
          </p>
